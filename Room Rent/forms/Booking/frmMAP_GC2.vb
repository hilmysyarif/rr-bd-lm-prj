    ...
    Sub LoadRooms()
        ...
        For Each dr As DataRow In objRooms.Selection().Rows
            If dr("Room").ToString <> "ESCORT" Then
                grpRoom_GC.Controls("btnRoom" & Val(dr("Sl")).ToString("00")).Tag = dr("Room").ToString & ":" & dr("FullName").ToString & ":" & grpRoom_GC.Controls("btnRoom" & Val(dr("Sl")).ToString("00")).Tag.ToString.Split(":")(2).Trim
                grpRoom_GC.Controls("btnRoom" & Val(dr("Sl")).ToString("00")).Text = dr("FullName").ToString
            End If
        Next
    End Sub
