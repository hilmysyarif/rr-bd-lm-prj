...
Imports System.IO
Public Class cls_Temp_tblVouchers
Inherits Database_Table_code_class_tblVouchers
...
Select isnull(Sum (isnull(VoucherAmount,0)),0)  as [UsedValue]
from 
tblBookingPayments 
where VoucherID=@VoucherID
    ...
    Function VoucherAmountUser(ByVal VoucherID As Integer, Optional ByRef conn As SqlConnection = Nothing, Optional ByRef transact As SqlTransaction = Nothing) As Double
        Dim UsedAmount As Double = 0
        ...
        If conn Is Nothing Then
            ...
            isDisposeConn = True
        End If
        Dim comFind As New SQLCommand(tblVouchers_USED_AMOUNT, conn)
        If transact IsNot Nothing Then
            comFind.Transaction = transact
        End If
        comFind.Parameters.AddWithValue("@VoucherID", VoucherID)
        Try
            UsedAmount = comFind.ExecuteScalar
        Catch ex As Exception
