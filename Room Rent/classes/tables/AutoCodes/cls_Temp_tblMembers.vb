...
Imports System.IO
Public Class cls_Temp_tblMembers
Inherits Database_Table_code_class_tblMembers
    ...
    Public ReadOnly Property tblMembers_select_report
        Get
...
SELECT 
      ...
      a.[Phone],
      a.[Address],
      a.[CreatedDate],b.TOTAIL_PAID , b.GST 
FROM [tblMembers] a
left outer join 
tblPayment b on a.sl =  b.Transc_ID and b.Transac_Type='MEMBERSHIP' and convert(varchar,a.CreatedDate,101) = convert(varchar,b.Transc_Time,101)
    ...
    Function Selection(Optional ByVal _selection_type As SelectionType = SelectionType.All, Optional ByVal _SelectString As String = "", Optional ByVal _params As List(Of SqlParameter) = Nothing, Optional ByVal _conn As SqlConnection = Nothing, Optional ByVal _transac As SqlTransaction = Nothing) As DataTable
        ...
        Select Case _selection_type
            ...
            Case SelectionType.Report
                comSelection.CommandText = tblMembers_select_report & IIf(_SelectString <> "", IIf(_SelectString.Trim.StartsWith("AND"), _SelectString, " AND " & _SelectString), "")
