...
Imports System.IO
Public Class cls_Temp_tblInstant
Inherits Database_Table_code_class_tblInstant

    Enum SelectionType
        All = 0
        report = 1
    End Enum
    Public ReadOnly Property tblInstant_select_report
        Get
...
SELECT 
      ...
      [DoorName],
      [Qty],
      convert(decimal(18,2), [DoorCharge]) [DoorCharge] ,
      convert(decimal(18,2), [TotalAmount]) [TotalAmount]--,
      --[Status],
      --[MemoNo],
      --[UserId]
FROM [tblInstant]
    ...
    Function Selection(Optional ByVal _selection_type As SelectionType = SelectionType.All, Optional ByVal _SelectString As String = "", Optional ByVal _params As List(Of SqlParameter) = Nothing, Optional ByVal _conn As SqlConnection = Nothing, Optional ByVal _transac As SqlTransaction = Nothing) As DataTable
        ...
        Select Case _selection_type
            ...
            Case SelectionType.report
                comSelection.CommandText = tblInstant_select_report & IIf(_SelectString <> "", IIf(_SelectString.Trim.StartsWith("AND"), _SelectString, " AND " & _SelectString), "")
